...
var skinUrl = "http://www.yyyg.com:8082";
	...
	RightTool: {
		...
		switchMiniLogin: function() {
			...
			$("#btnWXLogin").click(function() {
				$.cookie("qFromUrl", escape("http://www.yyyg.com"));
				if (_IsIE && _IeVersion == 6) {
			...
			h.draggable({
				...
				stop: function(l, n) {
					...
					if (k < m - i) {
						...
						var p = o.val();
						GetJPData("https://passport.1yyg.com", "getVcChar", "key=" + p,function(q) {
							if (q.state == 1) {
								f.resetVcCode();
		...
		rightToolEvent: function() {
			...
			e.RightCartMainObj.on("mouseenter", function() {
				e.isHideCartPanelActive = false;
				if (d != null) {
					clearTimeout(d)
				}
			...
			e.RightToolObj.on("mouseenter", function() {
				if (a != null) {
					clearTimeout(a);
				}
			}).on("mouseleave", function() {
			...
			e.RightLoginObj.on("mouseenter", function() {
				if (d != null) {
					clearTimeout(d);
				}
				if (a != null) {
...
/*$(function() {
	Base.getScript(skinUrl + "/resources/js/common/common.js?date=16071500", function() {
		Base.getScript(skinUrl + "/resources/js/common/cart_common.js?date=160513", YG.Bottom.BindEvents)
	});
});*/