...
function GetAjaxPageNationEx2(b, j, a, l, g, h, k) {
	...
	if (h) {
		e += '<li class="total_page">页次<i>' + l + "/" + c + "</i>&nbsp;&nbsp;共<i>" + b + "</i>条记录</li>"
	}
	...
	if (l == 1) {
		if (k) {
			e += '<li class="prev_page page_curgray"><a><i><</i>上一页</a></li>'
		}
	} else {
		e += '<li class="prev_page"><a href="javascript:gotoClick();" onclick="javascript:return ' + g + "(" + ((l - 2) * j + 1) + "," + (l - 1) * j + ');"><i><</i>上一页</a></li>'
	}
	if (f > 1) {
		e += '<li><a href="javascript:gotoClick();" onclick="javascript:return ' + g + "(1," + j + ');">1</a></li><li>…</li>'
	}
	for (var d = f; d <= m; d++) {
		if (d != l) {
	...
	if (m < c) {
		e += '<li>…</li><li><a href="javascript:gotoClick();" onclick="javascript:return ' + g + "(" + ((c - 1) * j + 1) + "," + c * j + ');">' + c + "</a></li>"
	}
	if (l < c) {
		e += '<li class="next_page"><a href="javascript:gotoClick();" onclick="javascript:return ' + g + "(" + (l * j + 1) + "," + (l + 1) * j + ');">下一页<i>></i></a></li>'
	} else {
		if (k) {
			e += '<li class="prev_page page_curgray"><a>下一页<i>></i></a></li>'
		}
	}
	e += "</ul>";
	return e
