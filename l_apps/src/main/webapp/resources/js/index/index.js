var skinDomain = "http://www.yyyg.com:8082";
...
(function(b, c) {
	function a(q, t) {
		...
		q.on("click", "#ctrl_prev", function(u) {
			g();
		}).on("click", "#ctrl_next", function(u) {
			l();
		}).on("click", "#handle_box li", function(v) {
...
$(document).ready(function() {
	...
	/*Base.getScript(skinDomain + "/resources/js/common/ajax_page.js?v=141103", function() {
		Base.getScript(skinDomain + "/resources/js/common/parabola.js?v=160615", YG.www_Index.BindEvents)
	});*/
});
YG.www_Index = {
	BindEvents: function() {
		...
		var u = function() {
			...
			GetJPData("http://poster.1yyg.com", A, "ID=2", function(D) {
				var E = "";
				if (D.state == 0) {
					var C = D.listItems;
...
//		u();
		var guideObj = function() {
			var divGuide = $("#div_guide");
			var guidePre = $("#guide_pre");
			var guideNext = $("#guide_next");
			var ulGuide = $("#ul_guide").children("li");
			var aRightWhat = $("#what_1yyg");
			var whatAnimateTmp = 0;
			var guideTmp = 0;
			var C = false;
			var guideFun = function() {
				divGuide.show();
				if (ulGuide.length <= 1) {
					guidePre.hide();
					guideNext.hide()
					...
					if (guideTmp <= 0) {
						...
						if (guideTmp >= (ulGuide.length - 1)) {
							guideTmp= ulGuide.length - 1;
							guidePre.show();
							guideNext.hide();
						} else {
							guidePre.show();
				...
				ulGuide.eq(guideTmpJ).hide().fadeToggle();
				ulGuide.eq(guideTmp).siblings().hide();
			};
			var M = function() {
				divGuide.hide();
				guidePre.hide();
				guideNext.show();
				ulGuide.eq(guideTmpJ).show();
				ulGuide.eq(guideTmpJ).siblings().hide();
				P = 0;
				if (N != null) {
					clearInterval(N)
				}
			...
			guideNext.bind("click", function() {
				I()
			});
			guidePre.bind("click", function() {
				J--;
				L()
			});
			var whatYgAnimateOpen = function() {
				if (C) {
					return
				}
				C = true;
			...
			var whatYgAnimateClose = function() {
				if (!C) {
					return
				}
				C = false;
			...
			aRightWhat.bind("click",function() {
				if (whatAnimateTmp == 0) {
					whatYgAnimateOpen();
				} else {
					whatYgAnimateClose();
		...
		var n = function() {
			...
			var C = false;
			var E = function() {
				GetJPData("http://api.1yyg.com", "GetStartRaffleAllList", "time=" + F, function(H) {
					if (H.errorCode == 0) {
						G(H, B)
					}
					if (A == null) {
		...
		var e = function() {
			var A = $("#divPostRec");
			var z = $("#ul_PostList");
			GetJPData("http://api.1yyg.com", "getRecPostList", "", function(G) {
				if (G.code == 0) {
					var K = G.listItems;
					var H = K.length;
